...
async function deployContracts() {
    let registerationTime = parseInt(document.getElementById('registerationTime').value);
    ...
    registerationTime = convertToSeconds(registerationTime, registerationTimeFormat);
    ...
    const userProfileDeployed = await userProfileContract.deploy({
    ...
    const ballotDeployed = await ballotContract.deploy({
        data: ballotBytecode,
        arguments: [registrationEndTime, votingEndTime, maxCandidates, userProfileDeployed.options.address]
    }).send({
        from: chairperson,
        gas: '3000000'
    });
    ...
    localStorage.setItem('userProfileAddress', userProfileDeployed.options.address);
    ...
    disableForm();
    displayContractAddresses(userProfileDeployed.options.address, ballotDeployed.options.address);
...
window.onload = function() {
    ...
    if (userProfileAddress && ballotAddress) {
        ...
        document.getElementById('registerationTime').value = localStorage.getItem('registerationTime');
        document.getElementById('votingTime').value = localStorage.getItem('votingTime');
        document.getElementById('maxCandidates').value = localStorage.getItem('maxCandidates');
